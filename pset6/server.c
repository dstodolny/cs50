...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            char method[bp - pline + 1];
            strncpy(method, pline, bp - pline);
            method[bp - pline] = '\0';
            pline = bp + 1;
            ...
            char target[bp - pline + 1];
            strncpy(target, pline, bp - pline);
            target[bp - pline] = '\0';
            pline = bp + 1;
            ...
            char version[bp - pline + 1];
            strncpy(version, pline, bp - pline);
            version[bp - pline] = '\0';
            ...
            // parse absolute path and query    
            char absolute[strlen(target) + 1];
            char query[strlen(target) + 1];
            char *ptarget = target;
            
            bp = strchr(target, '?');
            if (bp != NULL)
            {
                strncpy(absolute, ptarget, bp - ptarget);
                absolute[bp - ptarget] = '\0';
                ptarget = bp + 1;
                strcpy(query, ptarget);
            }
            else
            {
                strcpy(absolute, ptarget);
                strcpy(query, "");
            }
            
            if (strchr(absolute, '.') == NULL)
            {
                error(501);
                continue;
            }
            ...
            // done in parse absolute path

            // concatenate root and absolute-path
            char path[strlen(root) + strlen(absolute) + 1];
            char* ppath = path;
            strcpy(ppath, root);
            strcpy(ppath + strlen(root), absolute);
            path[strlen(root) + strlen(absolute)] = '\0';
            ...
            bp = strchr(path, '.');
            char extension[strlen(bp)];
            strcpy(extension, bp + 1);
            extension[strlen(bp) - 1] = '\0';
